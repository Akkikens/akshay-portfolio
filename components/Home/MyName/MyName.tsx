...
export default function MyName({ finishedLoading = false }: Props) {
  ...
  return (
    <section id="home" className="bg-gradient-to-br from-AAprimary via-AAprimary to-MobileNavBarColor text-AAtext relative overflow-hidden" aria-label="Introduction">
      ...
      <motion.div 
        className="absolute top-1/4 right-1/4 w-96 h-96 bg-AAsecondary/10 rounded-full blur-3xl"
        animate={{
          scale: [1, 1.2, 1],
          x: [0, 50, 0],
          y: [0, -30, 0],
      ...
      />
      <motion.div 
        className="absolute bottom-1/4 left-1/4 w-96 h-96 bg-AAaccent/10 rounded-full blur-3xl"
        animate={{
          scale: [1, 1.3, 1],
          x: [0, -50, 0],
          y: [0, 30, 0],
      ...
      {particles.map((particle, i) => (
        <motion.div
          key={i}
          className="absolute w-1 h-1 bg-AAsecondary/30 rounded-full"
          style={{
            left: particle.left,
            top: particle.top,
          }}
